...
async def chat_member_update_handler(client: Client, message: ChatMemberUpdated):
    ...
    # --- THIS IS THE FIX ---
    # Jab user request bhejta hai, tab status 'RESTRICTED' (pending) hota hai.
    # Humein use tab tak remove nahi karna hai.
    if message.new_chat_member.status == enums.ChatMemberStatus.RESTRICTED:
        return # User abhi bhi pending hai, kuch nahi karna.
    
    # Agar status RESTRICTED nahi hai (matlab MEMBER, LEFT, etc.),
    # toh unka request resolve ho chuka hai (approved, declined, ya cancelled).
    # Ab hum unhe pending list se hata sakte hain.
    try:
