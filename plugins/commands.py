...
from info import (
    ...
    SHORTENER_API2, LOG_API_CHANNEL, TWO_VERIFY_GAP, 
    
    # --- YEH IMPORT ADD KIYA GAYA ---
    DEFAULT_VERIFY_TIME,
    # --- IMPORT KHATAM ---
...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        verify_time_seconds = settings.get('verify_time', DEFAULT_VERIFY_TIME)
        ...
        try:
            if user_data["last_verified"] > user_data["second_time_verified"]:
                ...
                key_to_update = "second_time_verified"
                completed_step_num = 2
                msg_template = script.SECOND_VERIFY_COMPLETE_TEXT
            else:
                # Link 2 aakhri baar use hua tha, iska matlab Link 1 dikhaya gaya tha.
                # Isliye, 'last_verified' ko update karein.
                key_to_update = "last_verified"
                completed_step_num = 1
                msg_template = script.VERIFY_COMPLETE_TEXT
        except Exception:
        ...
        # --- YEH HAI AAPKA NAYA LOGIC ---

        # 1. Jo step poora hua, uska timestamp update karo
        current_time = datetime.now(tz=ist_timezone)  
        await db.update_notcopy_user(user_id, {key_to_update: current_time})
        ...
        if completed_step_num == 1:
            # --- SCENARIO 1: User ne V1 poora kiya ---
            # User ko 'VERIFY_COMPLETE_TEXT' *nahi* dikhana hai.
            # User ko seedha V2 ka link dikhana hai.
            
            # Naya V2 link generate karo
            new_verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
            await db.create_verify_id(user_id, new_verify_id)
            temp.CHAT[user_id] = grp_id
            
            # Hamesha V2 link (is_second_shortener=True)
            verify_url = await get_shortlink(
                f"https://telegram.me/{temp.U_NAME}?start=notcopy_{user_id}_{new_verify_id}_{file_id}", 
                grp_id, 
                is_second_shortener=True
            )
            
            buttons = [[
                InlineKeyboardButton(text="✅️ ᴠᴇʀɪғʏ (Step 2/2) ✅️", url=verify_url),
                InlineKeyboardButton(text="⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url=settings['tutorial'])
            ],[
                InlineKeyboardButton("😁 ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪOɴ - ɴᴏ ɴᴇᴇᴅ ᴛᴏ ᴠᴇʀɪғʏ 😁", callback_data='buy_premium')
            ]]
            reply_markup = InlineKeyboardMarkup(buttons)
            
            # User ko 'SECOND_VERIFICATION_TEXT' msg bhejho
            await m.reply_photo(
                photo=(VERIFY_IMG),
                caption=script.SECOND_VERIFICATION_TEXT.format(message.from_user.mention, get_status()),
            ...
            # --- SCENARIO 2: User ne V2 poora kiya ---
            # User ko "Click here to get file" NAHI dikhana hai.
            # Seedha FSUB 4 (Aakhri Step) check karo.
            ...
            if not await db.has_premium_access(user_id) and AUTH_CHANNEL_4:
                ...
                if fsub_4_status == "NOT_JOINED":
                    try:
                        ...
                        # "Try Again" link ko original file_id se set karo
                        original_start_command = f"file_{grp_id}_{file_id}"
                        
                        btn = [
                            [InlineKeyboardButton(f"{AUTH_CHANNEL_4_TEXT}", url=invite_link.invite_link)],
                            [InlineKeyboardButton("Try Again ♻️", url=f"https://t.me/{temp.U_NAME}?start={original_start_command}")]
                        ]
                        
                        # User ko FSub-4 ka message bhejho
                        await m.reply_photo(
                            photo=(VERIFY_IMG),
                        ...
                        logger.error(f"FSub 4 link generation error in notcopy: {e}")
                        # Error aaye toh file bhej do (fail-safe)
                        pass 
            
            # Agar FSub-4 OFF hai ya user pehle se joined hai,
            # Toh use file lene ke liye "Click here" wala link bhejho
            # (Kyunki V2 ke baad FSub-4 hi aakhri check tha)
            ...
            # Yahan hum 'SECOND_VERIFY_COMPLETE_TEXT' ka istemaal kar rahe hain
            caption = msg_template.format(message.from_user.mention, readable_access_time) 
            caption += f"\n\n{access_msg}"
    ...
    # --- YEH POORA BLOCK AAPKE NAYE LOGIC SE REPLACE KIYA GAYA HAI ---
    if not await db.has_premium_access(user_id):
        ...
        if not settings.get("is_verify", IS_VERIFY):
            # Verification OFF hai, user ko FSub-4 check par bhejho
            pass
        else:
            # Verification ON hai
            
            # 1. Admin ka set kiya hua main expiry time (Default: 24 ghante)
            main_expiry_seconds = settings.get('verify_time', DEFAULT_VERIFY_TIME) 
            
            # 2. V1 aur V2 ke beech ka button gap time (Default: 0 seconds)
            # YEH 'TWO_VERIFY_GAP' HAI, JO 'info.py' MEIN 0 HAI
            button_gap_seconds = TWO_VERIFY_GAP 
            
            # Check karein ki V1 (main access) valid hai ya nahi (24 ghante ke hisaab se)
            is_v1_valid = await db.is_user_verified(user_id, main_expiry_seconds) 
            
            if not is_v1_valid:
                # --- LOGIC 1: V1 EXPIRE HO GAYA HAI (YA PEHLI BAAR AAYA HAI) ---
                # User ka main access (24 ghante) expire ho gaya hai.
                # Hamesha V1 link dikhao.
                
                verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
                await db.create_verify_id(user_id, verify_id)
                temp.CHAT[user_id] = grp_id
                # Hamesha 'is_second_shortener=False' (V1 link)
                ...
                buttons = [[
                    InlineKeyboardButton(text="✅️ ᴠᴇʀɪғʏ (Step 1/2) ✅️", url=verify),
                    InlineKeyboardButton(text="⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url=settings['tutorial'])
                ],[
                    InlineKeyboardButton("😁 ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪOɴ - ɴᴏ ɴᴇᴇᴅ ᴛᴏ ᴠᴇʀɪғʏ 😁", callback_data='buy_premium')
                ]]
                reply_markup=InlineKeyboardMarkup(buttons)            
                msg = script.VERIFICATION_TEXT # Hamesha V1 text
                ...
                await d.delete()
                return # User ko file mat bhejho
            
            # Agar V1 valid hai (24 ghante ke andar), toh V2 check karo
            
            # 'use_second_shortener' check karega ki V2 ka timestamp V1 se purana hai ya nahi
            # Hum yahan 'button_gap_seconds' (0 seconds) bhej rahe hain
            needs_v2 = await db.use_second_shortener(user_id, button_gap_seconds)
            
            if needs_v2:
                # --- LOGIC 2: V1 VALID HAI, LEKIN V2 PENDING HAI ---
                # User ne V1 kar liya hai, lekin V2 abhi tak nahi kiya.
                # V2 link dikhao.
                
                verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
                await db.create_verify_id(user_id, verify_id)
                temp.CHAT[user_id] = grp_id
                # Hamesha 'is_second_shortener=True' (V2 link)
                verify = await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start=notcopy_{user_id}_{verify_id}_{file_id}", grp_id, is_second_shortener=True)
                buttons = [[
                    InlineKeyboardButton(text="✅️ ᴠᴇʀɪғʏ (Step 2/2) ✅️", url=verify),
                    InlineKeyboardButton(text="⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url=settings['tutorial'])
                ],[
                    InlineKeyboardButton("😁 ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪOɴ - ɴᴏ ɴᴇᴇᴅ ᴛᴏ ᴠᴇʀɪғʏ 😁", callback_data='buy_premium')
                ]]
                reply_markup=InlineKeyboardMarkup(buttons)            
                msg = script.SECOND_VERIFICATION_TEXT # Hamesha V2 text
                d = await m.reply_text(
                    text=msg.format(message.from_user.mention, get_status()),
                    protect_content = False,
                    reply_markup=reply_markup,
            ...
            # Agar user yahan tak pohochta hai, matlab:
            # 1. V1 valid hai (24h ke andar)
            # 2. V2 bhi valid hai (0s gap ke andar)
            # Isliye, user ko file milni chahiye (FSub-4 check ke baad)
            pass
    ...
    try:
        if (
            ...
            if new_count >= REFERRAL_TARGET:
                ...
                # --- YEH ASLI FIX HAI ---
                # 'db.update_one' ko 'db.update_user_data' se replace kiya gaya hai
                # Yeh 'update_notcopy_user' wale pattern ko follow karta hai
