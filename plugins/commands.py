...
from info import (
    ...
    # --- YEH V3 IMPORT ADD KIYA GAYA ---
    SHORTENER_WEBSITE3, SHORTENER_API3,
    # --- IMPORT KHATAM ---

    LOG_API_CHANNEL, TWO_VERIFY_GAP, 
    DEFAULT_VERIFY_TIME,
    QR_CODE, DELETE_TIME, 
...
async def start(client:Client, message): 
    ...
    # --- YEH HAI 'NOTCOPY' (VERIFICATION) WALA FIX (UPDATED FOR V3 + NAIVE/AWARE FIX) ---
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        # --- V3 LOGIC CHECK (NAIVE/AWARE FIX) ---
        def make_aware(dt):
            """Agar datetime 'naive' hai, toh use 'aware' banata hai."""
            if dt.tzinfo is None:
                # Maan lo ki DB mein save time IST hi tha
                return ist_timezone.localize(dt)
            # Agar pehle se aware hai, toh waisa hi return karo
            return dt.astimezone(ist_timezone)

        try:
            v1_time = make_aware(user_data["last_verified"])
            v2_time = make_aware(user_data["second_time_verified"])
            
            # Get v3, use default if missing
            v3_default = datetime(2021, 5, 17, 0, 0, 0, tzinfo=ist_timezone)
            v3_time = make_aware(user_data.get("third_time_verified", v3_default))

        except Exception as e:
            logger.error(f"Error making datetimes aware in notcopy: {e}")
            # Failsafe (yeh pehle se 'aware' hain)
            v1_time = datetime(2019, 5, 17, 0, 0, 0, tzinfo=ist_timezone)
            v2_time = datetime(2020, 5, 17, 0, 0, 0, tzinfo=ist_timezone)
            v3_time = datetime(2021, 5, 17, 0, 0, 0, tzinfo=ist_timezone)
        # --- FIX KHATAM ---

        if v1_time < v2_time:
            # SCENARIO: V1 LINK CLICK HUA
            key_to_update = "last_verified"
            completed_step_num = 1
            next_step_num = 2
            next_step_text = "Step 2/3"
            next_script_text = script.SECOND_VERIFICATION_TEXT
        
        elif v2_time < v3_time:
            # SCENARIO: V2 LINK CLICK HUA
            key_to_update = "second_time_verified"
            completed_step_num = 2
            next_step_num = 3
            next_step_text = "Step 3/3"
            next_script_text = script.THIRD_VERIFICATION_TEXT
        
        else:
            # SCENARIO: V3 LINK CLICK HUA
            key_to_update = "third_time_verified"
            completed_step_num = 3
            next_step_num = None # Koi agla step nahi
        # --- LOGIC FIX KHATAM ---
        ...
        if next_step_num is not None:
            # --- SCENARIO 1 or 2: User ne V1 ya V2 poora kiya ---
            # Agle step (V2 ya V3) ka link dikhana hai.
            ...
            verify_url = await get_shortlink(
                ...
                grp_id, 
                step_number=next_step_num # Step 2 ya 3
            )
            
            buttons = [[
                InlineKeyboardButton(text=f"✅️ ᴠᴇʀɪғʏ ({next_step_text}) ✅️", url=verify_url),
            ...
            await m.reply_photo(
                photo=(VERIFY_IMG),
                caption=next_script_text.format(message.from_user.mention, get_status()),
            ...
            # --- SCENARIO 3: User ne V3 (aakhri) poora kiya ---
            # Ab FSub-4 check karo
            ...
            # Agar FSub-4 OFF hai ya user joined hai:
            # User ko file lene ke liye link bhejho
            ...
            caption = script.THIRD_VERIFY_COMPLETE_TEXT.format(message.from_user.mention, readable_access_time) 
            caption += f"\n\n{access_msg}"
    ...
    # --- YEH POORA BLOCK NAYA V3 LOGIC HAI (SAHI FIX KE SAATH) ---
    if not await db.has_premium_access(user_id):
        ...
        if not settings.get("is_verify", IS_VERIFY):
            # Verification OFF hai
            pass
        else:
            # Verification ON hai
            
            # 1. Admin ka set time (e.g., 10 min)
            main_expiry_seconds = settings.get('verify_time', DEFAULT_VERIFY_TIME) 
            
            # 2. Check V1 status against main timer
            ...
            if not is_v1_valid:
                ...
                verify = await get_shortlink(
                    ...
                    grp_id, 
                    step_number=1 # V1 link
                ) 
                buttons = [[
                    InlineKeyboardButton(text="✅️ ᴠᴇʀɪғʏ (Step 1/3) ✅️", url=verify),
                    InlineKeyboardButton(text="⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url=settings['tutorial'])
                ],[
                    InlineKeyboardButton("😁 ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪOɴ - ɴᴏ ɴᴇᴇᴅ ᴛO ᴠᴇʀɪғʏ 😁", callback_data='buy_premium')
                ]]
                reply_markup=InlineKeyboardMarkup(buttons)            
            ...
            # 3. Check V2 status against *main timer*
            #    YAHI ASLI FIX HAI
            is_v2_valid = await db.is_user_verified_second(user_id, main_expiry_seconds)
            
            if not is_v2_valid:
                # --- LOGIC 2: V1 VALID, V2 EXPIRED ---
                verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
                await db.create_verify_id(user_id, verify_id)
                temp.CHAT[user_id] = grp_id
                ...
                verify = await get_shortlink(
                    f"https://telegram.me/{temp.U_NAME}?start=notcopy_{user_id}_{verify_id}_{file_id}", 
                    grp_id, 
                    step_number=2 # V2 link
                )
                buttons = [[
                    InlineKeyboardButton(text="✅️ ᴠᴇʀɪғʏ (Step 2/3) ✅️", url=verify),
                    InlineKeyboardButton(text="⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url=settings['tutorial'])
                ],[
                    InlineKeyboardButton("😁 ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪOɴ - ɴᴏ ɴᴇᴇᴅ ᴛO ᴠᴇʀɪғʏ 😁", callback_data='buy_premium')
                ]]
            ...
            # 4. Check V3 status against *main timer*
            #    YAHI ASLI FIX HAI
            is_v3_valid = await db.is_user_verified_third(user_id, main_expiry_seconds)

            if not is_v3_valid:
                # --- LOGIC 3: V1, V2 VALID, V3 EXPIRED ---
                verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
                await db.create_verify_id(user_id, verify_id)
                temp.CHAT[user_id] = grp_id
                ...
                verify = await get_shortlink(
                    f"https://telegram.me/{temp.U_NAME}?start=notcopy_{user_id}_{verify_id}_{file_id}", 
                    grp_id, 
                    step_number=3 # V3 link
                )
                buttons = [[
                    InlineKeyboardButton(text="✅️ ᴠᴇʀɪғʏ (Step 3/3) ✅️", url=verify),
                    InlineKeyboardButton(text="⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url=settings['tutorial'])
                ],[
                    InlineKeyboardButton("😁 ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪOɴ - ɴᴏ ɴᴇᴇᴅ ᴛO ᴠᴇʀɪғʏ 😁", callback_data='buy_premium')
                ]]
...
# --- YEH NAYA COMMAND ADD KIYA GAYA HAI ---
...
async def set_shortner_3(c, m):
    ...
    try:
        ...
        await m.reply_text(f"<b><u>✅ sᴜᴄᴄᴇssꜰᴜʟʟʏ ʏᴏᴜʀ 3ʀᴅ sʜᴏʀᴛɴᴇʀ ɪs ᴀᴅᴅᴇᴅ</u>\n\nᴅᴇᴍᴏ - {SHORT_LINK}\n\nsɪᴛᴇ - `{URL}`\n\nᴀᴘɪ - `{API}`</b>", quote=True)
        user_id = m.from_user.id
        user_info = f"@{m.from_user.username}" if m.from_user.username else f"{m.from_user.mention}"
        link = (await c.get_chat(m.chat.id)).invite_link
        grp_link = f"[{m.chat.title}]({link})"
...
<u>✅️ 3ʀᴅ ᴠᴇʀɪꜰʏ sʜᴏʀᴛɴᴇR ɴᴀᴍᴇ/ᴀᴘɪ</u>
