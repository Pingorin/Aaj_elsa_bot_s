...
# Naya Text V3 ke liye (Script.py mein add kar sakte hain, ya yahin rakhein)
script.THIRD_VERIFICATION_TEXT = script.SECOND_VERIFICATION_TEXT # Placeholder
...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith("get_referral_"):
        # ... [Aapka referral code (jaisa tha waisa hi)] ...
        try:
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        # Naya Format: notcopy_{level}_{userid}_{verify_id}_{file_id}
        try:
        ...
        if level == 1:
            ...
            caption = "<b>✅ Verification Successful! (3/3) ✅</b>\n\nHey {0}!".format(message.from_user.mention)
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        # ... [Aapka group start message logic (jaisa tha waisa hi)] ...
        status = get_status()
    ...
    if len(message.command) != 2:
        # ... [Aapka PM start message (jaisa tha waisa hi)] ...
        buttons = [[
    ...
    if AUTH_CHANNEL or AUTH_CHANNEL_2 or AUTH_CHANNEL_3:
        # ... [Aapka FSub logic (jaisa tha waisa hi)] ...
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help", "buy_premium"]:
        # ... [Aapka Premium logic (jaisa tha waisa hi)] ...
        ...
        if settings.get("is_verify", IS_VERIFY):
            ...
            if show_link:
                ...
                if shortener_level == 1:
                    ...
                    msg = script.THIRD_VERIFICATION_TEXT # (Aapko yeh script.py mein add karna hoga)
    ...
    if not await db.has_premium_access(user_id) and AUTH_CHANNEL_4:
        # ... [Aapka FSub 4 logic (jaisa tha waisa hi)] ...
...
# --- '/set_verify_time' COMMAND (Full Access DURATION) ---
...
# --- NAYA/UPDATED COMMAND '/set_verify_gap_1' (Gap 1 DURATION) ---
...
# --- NAYA COMMAND '/set_verify_gap_2' (Gap 2 DURATION) ---
...
# --- 'COMBINED HANDLER' (Referral & Fsub Cleanup) ---
@Client.on_chat_member_updated()
async def combined_chat_member_handler(client: Client, member: ChatMemberUpdated):
    # ... [Aapka referral aur fsub logic (jaisa tha waisa hi)] ...
