...
from info import (
    ...
    # Note: Global Auth variables are still imported for defaults, 
    # but code now uses DB values via utils.
...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith("get_referral_"):
        try:
            ...
            current_count = user_data.get('referral_count', 0) if user_data else 0
    ...
    # --- PREPARE FOR FSUB CHECK ---
    # Extract Group ID if present in the start command (e.g. file_123_456 or allfiles_123-456)
    check_grp_id = None
    try:
        if len(message.command) == 2:
            if "file_" in message.command[1]:
                # file_{grp_id}_{file_id}
                parts = message.command[1].split("_")
                if len(parts) >= 3:
                    check_grp_id = int(parts[1])
            elif "allfiles_" in message.command[1]:
                # allfiles_{grp_id}-{msg_id}
                data_str = message.command[1].replace("allfiles_", "")
                if "-" in data_str:
                    grp_id_str, _ = data_str.split("-", 1)
                    check_grp_id = int(grp_id_str)
    except:
        check_grp_id = None

    # --- 3-CHANNEL FSUB LOGIC (UPDATED FOR GROUP SETTINGS) ---
    # Pass check_grp_id to the util function. It returns statuses AND specific IDs
    status_1, status_2, status_3, id_1, id_2, id_3 = await check_fsub_status(client, message.from_user.id, check_grp_id)
    
    # Check if ANY channel is set (Global or Custom)
    if id_1 or id_2 or id_3:
        ...
        if not all_joined:
            ...
            # Slot 1 (Request/Advanced)
            if id_1 and status_1 == "NOT_JOINED":
                try:
                    link_1 = await client.create_chat_invite_link(id_1, creates_join_request=True)
                    fsub_row_1.append(InlineKeyboardButton("Request Channel 1 🔗", url=link_1.invite_link))
                except Exception as e:
                    logger.error(f"Channel 1 ({id_1}) link error: {e}")
            
            # Slot 2 (Request/Advanced)
            if id_2 and status_2 == "NOT_JOINED":
                try:
                    link_2 = await client.create_chat_invite_link(id_2, creates_join_request=True)
                    fsub_row_1.append(InlineKeyboardButton("Request Channel 2 🔗", url=link_2.invite_link))
                except Exception as e:
                    logger.error(f"Channel 2 ({id_2}) link error: {e}")
            
            if fsub_row_1: btn.append(fsub_row_1)
            fsub_row_2 = []
            
            # Slot 3 (Normal)
            if id_3 and status_3 == "NOT_JOINED":
                try:
                    # Try export link first (Faster for public/existing)
                    invite_link_3 = await client.export_chat_invite_link(id_3)
                    fsub_row_2.append(InlineKeyboardButton("Join Channel 3 🔗", url=invite_link_3))
                except Exception as e:
                    # Fallback to create link
                    try:
                        link_3_obj = await client.create_chat_invite_link(id_3)
                        fsub_row_2.append(InlineKeyboardButton("Join Channel 3 🔗", url=link_3_obj.invite_link))
                    except Exception as e2:
                        logger.error(f"Channel 3 ({id_3}) link error: {e2}")
                        # Fallback for Username
                        if isinstance(id_3, str) and id_3.startswith("@"):
                            fsub_row_2.append(InlineKeyboardButton("Join Channel 3 🔗", url=f"https://t.me/{id_3.replace('@', '')}"))
    ...
    # --- FSUB 4 CHECK (UPDATED FOR GROUP SETTINGS) ---
    # check_grp_id was calculated above
    if not await db.has_premium_access(user_id):
        fsub_4_status, id_4 = await check_fsub_4_status(client, user_id, check_grp_id)
        
        if id_4 and fsub_4_status == "NOT_JOINED":
            try:
                invite_link = await client.create_chat_invite_link(id_4, creates_join_request=True)
                btn = [
...
async def settings(client, message):
    ...
    if settings is not None:
            buttons = [
                ...
                # --- UPDATED BUTTONS FOR NEW FEATURES ---
...
async def set_shortner(c, m):
    ...
    try:
        ...
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>...</b>", quote=True)
...
async def combined_chat_member_handler(client: Client, member: ChatMemberUpdated):
    try:
        # Use Global Constants for default cleaning
        # Ideally this should iterate all groups, but for now standard is fine.
