...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        # Check karte waqt custom time ka istemaal karein
        key = "second_time_verified" if await db.is_user_verified(user_id, verify_time_seconds) else "last_verified"
        ...
        # Purane 'msg' variable ko 'caption' mein replace karein
        caption = msg.format(message.from_user.mention) # Purana format hata diya
    ...
    if not await db.has_premium_access(user_id):
        ...
        # --- YEH HAI AAPKA CUSTOM TIME WALA FIX ---
        settings = await get_settings(grp_id)
        # Admin dwara set kiya gaya time (seconds mein) fetch karein
        verify_gap_seconds = settings.get('verify_time', TWO_VERIFY_GAP) 
        
        # Pehli verification ke liye 'verify_gap_seconds' ka istemaal karein
        user_verified = await db.is_user_verified(user_id, verify_gap_seconds) 
        
        # Doosri verification ke liye bhi 'verify_gap_seconds' ka istemaal karein
        is_second_shortener = await db.use_second_shortener(user_id, verify_gap_seconds)
        
        # Purana logic (ab 'user_verified' naye time ke hisaab se set hai)
