...
from info import (
    ...
    AUTH_CHANNEL, AUTH_CHANNEL_2, # <-- Dono channel imported hain
...
# <-- 'check_fsub' function imported hai
from utils import get_settings, save_group_settings, check_fsub, get_size, get_shortlink, is_check_admin, get_status, temp, get_readable_time
import re
import json
import base64
...
async def start(client:Client, message): 
    ...
    # --- YEH SEQUENTIAL FSUB LOGIC BILKUL SAHI HAI ---
    # (Aapka error `info.py` mein galat ID ya permissions na hone ke kaaran aa raha hai)
    fsub_pass, failed_channel = await check_fsub(client, message)
    
    if not fsub_pass:
        try:
            invite_link = await client.create_chat_invite_link(int(failed_channel), creates_join_request=True)
        except ChatAdminRequired:
            logger.error(f"Make sure Bot is admin in FSub channel: {failed_channel}")
            return
        ...
        await client.send_message(
            ...
            text="🙁 ғɪʀꜱᴛ ᴊᴏɪɴ ᴏᴜʀ ᴄʜᴀɴɴᴇʟ ᴛʜᴇɴ ʏᴏᴜ ᴡɪʟʟ ɢᴇᴛ ᴍᴏᴠɪᴇ.\n\nᴄʟɪᴄᴋ ᴊᴏɪɴ ɴᴏᴡ ʙᴜᴛᴛᴏɴ, ꜱᴇɴᴅ ʀᴇǫᴜᴇꜱᴛ, ᴀɴᴅ ᴛʜᴇɴ ᴄʟɪᴄᴋ 'ᴛʀʏ ᴀɢᴀɪɴ'.",
            reply_markup=InlineKeyboardMarkup(btn),
            parse_mode=enums.ParseMode.HTML
        )
        return
...
@Client.on_message(filters.command("checkadmin") & filters.user(ADMINS))
async def check_bot_status(client, message):
    msg = "<b>🤖 Bot Status Check:</b>\n\n"
    try:
        # Check Channel 1
        chat1 = await client.get_chat(AUTH_CHANNEL)
        member1 = await client.get_chat_member(AUTH_CHANNEL, "self")
        msg += f"<b>✅ Channel 1 ({chat1.title}):</b>\n"
        msg += f"   - Bot Status: <code>{member1.status}</code>\n"
        msg += f"   - Can Invite: <code>{member1.privileges.can_invite_users}</code>\n"
        msg += f"   - Can Manage Joins: <code>{member1.privileges.can_manage_join_requests}</code>\n\n"
    except Exception as e:
        msg += f"<b>❌ Channel 1 (ID: {AUTH_CHANNEL}):</b>\n   - Error: <code>{e}</code>\n\n"
    
    try:
        # Check Channel 2
        chat2 = await client.get_chat(AUTH_CHANNEL_2)
        member2 = await client.get_chat_member(AUTH_CHANNEL_2, "self")
        msg += f"<b>✅ Channel 2 ({chat2.title}):</b>\n"
        msg += f"   - Bot Status: <code>{member2.status}</code>\n"
        msg += f"   - Can Invite: <code>{member2.privileges.can_invite_users}</code>\n"
        msg += f"   - Can Manage Joins: <code>{member2.privileges.can_manage_join_requests}</code>\n"
    except Exception as e:
        msg += f"<b>❌ Channel 2 (ID: {AUTH_CHANNEL_2}):</b>\n   - Error: <code>{e}</code>\n"
...
# --- FIX: REFERRAL HANDLER MEIN KEYERROR AUR ATTRIBUTEERROR DONO FIX KIYE GAYE ---
...
async def welcome_handler(client: Client, member: ChatMemberUpdated):
    try:
        if (
            ...
            # FIX 1: KeyError ko fix kiya (referrer['id'] use kiya)
            ...
            # FIX 2: AttributeError ko fix kiya (fallback mention banaya)
            try:
                referrer_user = await client.get_users(referrer_id)
                referrer_mention = referrer_user.mention
            except Exception:
                referrer_name = referrer.get('first_name', f"User {referrer_id}")
            ...
            if new_count >= REFERRAL_TARGET:
                ...
                try:
                    await client.send_message(
                        chat_id=referrer_id,
                        # Fixed variable ka istemal karein
