...
async def start(client:Client, message): 
    ...
    # --- YEH POORA BLOCK NAYA V3 LOGIC HAI ---
    if not await db.has_premium_access(user_id):
        ...
        if not settings.get("is_verify", IS_VERIFY):
            ...
            button_gap_seconds = TWO_VERIFY_GAP # 0 seconds + grace period
            ...
            # V1 Valid hai. Ab V2 check karo.
            is_v2_valid = await db.is_user_verified_second(user_id, button_gap_seconds)
            ...
            # V1 aur V2 Valid hain. Ab V3 check karo.
            is_v3_valid = await db.is_user_verified_third(user_id, button_gap_seconds)

            if not is_v3_valid:
                # --- LOGIC 3: V1 & V2 VALID HAIN, LEKIN V3 PENDING HAI ---
                verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
                await db.create_verify_id(user_id, verify_id)
                temp.CHAT[user_id] = grp_id
