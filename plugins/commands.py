...
from info import (
    ...
    TWO_VERIFY_GAP,  # <-- Yeh default gap ke liye hai
    DEFAULT_VERIFY_DURATION, # <-- YEH ADD KIYA GAYA (24 hours default duration)
    QR_CODE, DELETE_TIME, 
...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        # YEH BADLA GAYA: Default ab 24 hours (DEFAULT_VERIFY_DURATION) hai
        verify_time_seconds = settings.get('verify_time', DEFAULT_VERIFY_DURATION)
    ...
    if not await db.has_premium_access(user_id):
        ...
        # --- YEH HAI NAYA VERIFICATION FIX (DURATION & GAP ALAG) ---
        settings = await get_settings(grp_id)
        # Verify 'DURATION' (default 24h)
        verify_duration_seconds = settings.get('verify_time', DEFAULT_VERIFY_DURATION) 
        # Verify 'GAP' (default from info.py)
        verify_gap_seconds = settings.get('verify_gap', TWO_VERIFY_GAP)
        
        # Pehli verification (Link 1) check karein (duration ke hisaab se)
        is_link_1_valid = await db.is_user_verified(user_id, verify_duration_seconds) 
        
        # Doosri verification (Link 2) check karein (duration ke hisaab se)
        is_link_2_valid = await db.is_user_verified_second(user_id, verify_duration_seconds)
        
        # Decide karein kaunsa link dikhana hai (Link 2 = True, Link 1 = False)
        # Yeh ab 'duration' aur 'gap' dono check karega
        is_second_shortener = await db.use_second_shortener(user_id, verify_duration_seconds, verify_gap_seconds)
...
<u>⏰ ᴠᴇʀɪꜰʏ ꜱᴇᴛᴛɪɴɢꜱ</u>
ᴅᴜʀᴀᴛɪᴏɴ - `{readable_duration}` (Access time)
ɢᴀᴘ - `{readable_gap}` (Time between links)
...
async def set_verify_time(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command poora nahi hai! (Yeh DURATION set karta hai)</b>\n\n"
            "Example:\n"
...
async def set_verify_gap(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command poora nahi hai! (Yeh do links ke beech ka GAP set karta hai)</b>\n\n"
            "Example:\n"
            "<code>/set_verify_gap 10 mins</code>\n"
            "<code>/set_verify_gap 1 hour</code>\n"
            "<code>/set_verify_gap 0</code> (Koi gap nahi)"
        )
    ...
    reply_text = (
        ...
        f"<b>Naya Gap Time:</b> <code>{time_string}</code> ({seconds} seconds)\n\n"
    )
    
    if seconds == 0:
        reply_text += "<i>Note: Aapne <code>0</code> set kiya hai. Ek link expire hote hi doosra dikh jayega.</i>"
