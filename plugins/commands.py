...
from info import (
    ...
    AUTH_CHANNEL_3, AUTH_CHANNEL_3_INVITE_LINK, 
    ...
    VERIFY_DURATION # <-- YEH IMPORT ADD KAREIN
)
from utils import (
...
async def start(client:Client, message): 
    ...
    # (Yeh poora code block sahi hai, koi badlaav nahi)
    ...
    # --- YEH HAI 'NOTCOPY' (VERIFICATION) WALA FIX ---
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        # Admin dwara set Expiry time ko fetch karein
        verify_duration_seconds = settings.get('verify_duration', VERIFY_DURATION) # Default 24h
        ...
        # --- YEH HAI ASLI LOGIC FIX: Decide karein kaunsa timestamp update karna hai ---
        user_data = await db.get_notcopy_user(user_id)
        
        # Pata lagao V1 valid hai ya V2
        is_v1_valid = await db.is_user_verified(user_id, grp_id, verify_duration_seconds)
        
        # V1-V2 Gap Time ko fetch karein
        verify_gap_seconds = settings.get('verify_time', TWO_VERIFY_GAP)
        
        # Check karein ki V2 dikhana chahiye tha ya nahi
        show_v2 = await db.use_second_shortener(user_id, grp_id, verify_gap_seconds, is_v1_valid)

        if show_v2:
            # Agar V2 dikhana chahiye tha, toh V2 ka link verify hua hai
            key = "second_time_verified"
            num = 2
            msg = script.SECOND_VERIFY_COMPLETE_TEXT
        else:
            # Varna V1 ka link verify hua hai
            key = "last_verified"
            num = 1
        ...
        # FIX (INDEXERROR): 'msg.format' ko dono arguments (user mention aur time) pass karein
        caption = msg.format(message.from_user.mention, get_readable_time(verify_gap_seconds)) # Purana message
        caption += f"\n\n{access_msg}" # Naya expiry message
    ...
    # --- YEH HAI AAPKA 3-CHANNEL FSUB LOGIC ---
    ...
    if not await db.has_premium_access(user_id):
        ...
        # --- YEH HAI NAYA VERIFICATION LOGIC ---
        settings = await get_settings(grp_id)
        verify_duration = settings.get('verify_duration', VERIFY_DURATION)
        verify_gap = settings.get('verify_time', TWO_VERIFY_GAP)
        
        is_v1_valid = await db.is_user_verified(user_id, grp_id, verify_duration)
        is_v2_valid = await db.is_user_verified_second(user_id, grp_id, verify_duration)
        
        # Pata lagao ki V2 dikhana hai ya V1
        show_v2 = await db.use_second_shortener(user_id, grp_id, verify_gap, is_v1_valid)
        
        # Agar V1 aur V2 dono valid hain, toh verification skip karo
        if is_v1_valid and is_v2_valid:
             pass # Verification skip, file dene aage badho
        
        # Agar V1 valid hai, lekin V2 invalid hai AUR V2 dikhane ka time ho gaya hai
        elif is_v1_valid and not is_v2_valid and show_v2:
            # Sirf V2 dikhao
            ...
            reply_markup=InlineKeyboardMarkup(buttons)            
            msg = script.SECOND_VERIFICATION_TEXT # V2 wala message
        ...
        # Agar V1 invalid hai (chahe V2 valid ho ya na ho)
        elif not is_v1_valid:
            # Sirf V1 dikhao
            verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
            await db.create_verify_id(user_id, verify_id)
            temp.CHAT[user_id] = grp_id
            ...
            reply_markup=InlineKeyboardMarkup(buttons)            
            msg = script.VERIFICATION_TEXT # V1 wala message
            d = await m.reply_text(
                text=msg.format(message.from_user.mention, get_status()),
                protect_content = False,
                reply_markup=reply_markup,
    ...
    if data and data.startswith("allfiles"):
        # (Yeh code block sahi hai, koi badlaav nahi)
...
async def set_shortner(c, m):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://{URL}?api={API}&url=https://telegram.dog/Aksbackup') as resp:
                if resp.status != 200:
...
async def set_shortner_2(c, m):
    ...
    if not await is_check_admin(c, grp_id, m.from_user.id):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪN ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    if len(m.text.split()) < 3:
        await m.reply("<b>Use this command like this - \n\n`/set_shortner_2 shortxlinks.com/st 7c480...`</b>")
        return
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await asyncio.sleep(1.2)
    await sts.delete()
    ...
    try:
        ...
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/aks_bot_support>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner_2 shortxlinks.com/st 7c480...`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)
...
# --- YEH HAI AAPKA PURANA/PEHLA 'set_time' (V1-V2 GAp ke liye) ---
# --- Maine iska naam 'set_gap_time' kar diya hai ---
@Client.on_message(filters.command('set_gap_time'))
async def set_gap_time(client, message):
    ...
    if not await is_check_admin(client, grp_id, message.from_user.id):
        return await message.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪN ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    try:
        # Yeh V1 aur V2 ke beech ka gap (seconds mein) set karta hai
        time = int(message.text.split(" ", 1)[1])
    except:
        return await message.reply_text("<b>Command Incomplete! Sirf seconds mein number daalein (jaise: <code>/set_gap_time 10</code>)</b>")   
    
    # Yeh 'verify_time' (V1-V2 gap) ko save karega
    await save_group_settings(grp_id, 'verify_time', time) 
    await message.reply_text(f"<b>✅ Successfully set V1-V2 gap time for {title}</b>\n\n<b>Gap Time:</b> <code>{time} seconds</code>")

# --- YEH HAI AAPKA NAYA '/set_verify_duration' COMMAND ---
# (Aapne galti se iska naam 'set_time' rakh diya tha)
@Client.on_message(filters.command('set_verify_duration'))
async def set_verify_duration(client, message):
    userid = message.from_user.id if message.from_user else None
    if not userid:
        return await message.reply("<b>ʏᴏᴜ ᴀʀᴇ ᴀɴᴏɴʏᴍᴏᴜꜱ ᴀᴅᴍɪN ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ...</b>")
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command poora nahi hai! \n\nIstemaal karein:</b> `/set_verify_duration 6 hours` ya `/set_verify_duration 1 day`"
            "\n\nDefault 24 ghante (24 hours) hai."
        )
    ...
    if seconds == 0 and time_string != "0" and not time_string.startswith("0"):
         return await message.reply_text(
            ...
            "Use karein: <code>min</code>, <code>hour</code>, <code>day</code>, <code>month</code>\n"
            "Example: <code>/set_verify_duration 12 hours</code>"
        )

    # Yeh 'verify_duration' (Total Expiry) ko save karega
    await save_group_settings(grp_id, 'verify_duration', seconds) 
    
    reply_text = (
        f"<b>✅ Verification expiry time {title} ke liye set ho gaya hai.</b>\n\n"
        f"<b>Naya Expiry Time:</b> <code>{time_string}</code>\n"
    )
