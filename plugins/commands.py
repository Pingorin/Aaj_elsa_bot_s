...
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, ChatMemberUpdated, CallbackQuery # <-- CallbackQuery import kiya
...
from info import (
    ...
    # --- FIX: NAYA VARIABLE ADD KIYA GAYA ---
    DEFAULT_VERIFY_EXPIRY 
    # ----------------------------------------
)
from utils import (
...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        try: # Added try-except for safe unpacking
            _, userid, verify_id, file_id = m.command[1].split("_", 3)
            user_id = int(userid)
        except ValueError:
            await message.reply("<b>Invalid verification link format or link expired (Data missing).</b>")
            return
            
        grp_id = temp.CHAT.get(user_id, 0)
        settings = await get_settings(grp_id)
        
        # --- FIX: Verification duration ke liye DEFAULT_VERIFY_EXPIRY use kiya ---
        verify_time_seconds = settings.get('verify_time', DEFAULT_VERIFY_EXPIRY)
        # -----------------------------------------------------------------------
        ...
        try:
            if user_data["last_verified"] > user_data["second_time_verified"]:
                # Link 1 aakhri baar use hua tha, iska matlab Link 2 dikhaya gaya था.
        ...
        # -----------------------------------------------------------
        # --- NAYA LOGIC: V1/V2 COMPLETE HONEY PAR KAUNSA ACCESS DENA HAI ---
        
        # Check karein ki V2 set hai ya nahi
        is_second_shortener_set = settings.get('shortner_two', SHORTENER_WEBSITE2) != SHORTENER_WEBSITE2
        ...
        # -----------------------------------------------------------
        # CASE 1: USER COMPLETED V1 (num=1) AND V2 IS SET (DUAL VERIFY MODE)
        if is_second_shortener_set and num == 1:
        
            # V1 complete hone ke baad V2 ka gap time shuru hoga.
            gap_seconds = get_seconds(TWO_VERIFY_GAP) # TWO_VERIFY_GAP is used as the delay
    
            # --- YEH HAI NAYA FIX (TWO_VERIFY_GAP = 0) ---
            if gap_seconds == 0:
                # Agar GAP 0 hai, toh file mat do. Seedha V2 ke liye prompt karo.
                
                verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
                await db.create_verify_id(user_id, verify_id)
                temp.CHAT[user_id] = grp_id
                
                # Forcefully V2 shortener ka use (is_second_shortener=True)
                ...
                buttons = [[
                    InlineKeyboardButton(text="✅️ ᴠᴇʀɪғʏ V2 ✅️", url=verify),
                ...
                # V2 ka verification message bhejein
                await m.reply_photo(
                    photo=(VERIFY_IMG), 
                    caption=script.SECOND_VERIFICATION_TEXT.format(m.from_user.mention, get_status()),
                ...
                # Agar GAP 0 nahi hai (normal flow), toh file + V2 button do.
                readable_gap_time = get_readable_time(gap_seconds)
                file_link = f"https://telegram.me/{temp.U_NAME}?start=file_{grp_id}_{file_id}"
                
                btn = [[
                    InlineKeyboardButton("✅ ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ɢᴇᴛ ꜰɪʟᴇ ✅", url=file_link),
                ],[
                    InlineKeyboardButton("✨ ᴠᴇʀɪғʏ ꜰᴏʀ ʟᴏNG ᴀᴄᴄᴇꜱꜱ (Step 2) ✨", callback_data=f'start_v2_check#{grp_id}#{file_id}') 
                ]]
                
                caption = script.VERIFY_COMPLETE_V1_SHORT_ACCESS.format(
                    m.from_user.mention, 
                    readable_gap_time
                )
        ...
        # -----------------------------------------------------------
        # CASE 2: USER COMPLETED V2 (num=2) OR V2 IS NOT SET (num=1, single verify)
        # Goal: Grant full (verify_time) long-term access.
        ...
        # --- FIX: 'reply_markup' ko yahan define karein (NameError fix) ---
        file_link = f"https://telegram.me/{temp.U_NAME}?start=file_{grp_id}_{file_id}"
        btn = [[
            InlineKeyboardButton("✅ ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ɢᴇᴛ ꜰɪʟᴇ ✅", url=file_link),
        ]]
        ...
        await m.reply_photo(
            ...
            reply_markup=reply_markup, # <--- ✅ AB YEH SAHI HAI
            parse_mode=enums.ParseMode.HTML
        )
        return 
    # --- 'NOTCOPY' FIX KHATAM ---
    ...
    if not await db.has_premium_access(user_id):
        ...
        # FIX: verify_gap_seconds (expiry duration) ke liye DEFAULT_VERIFY_EXPIRY use kiya
        verify_gap_seconds = settings.get('verify_time', DEFAULT_VERIFY_EXPIRY) 
...
async def set_verify_time(client, message):
    ...
    # --- FIX: TypeError se bachne ke liye 'await' hataya ---
    # Time string ko seconds mein convert karein
    seconds = get_seconds(time_string)
    # --- FIX KHATAM ---
...
# --- YEH HAI AAPKA V2 START CALLBACK HANDLER ---
@Client.on_callback_query(filters.regex("^start_v2_check"))
async def start_v2_flow(client: Client, query: CallbackQuery):
    try:
        _, grp_id, file_id = query.data.split("#")
        user_id = query.from_user.id
        grp_id = int(grp_id)

        settings = await get_settings(grp_id)
        
        # Check karein ki V1 ki verification abhi bhi TWO_VERIFY_GAP ke andar hai
        verify_gap_seconds = get_seconds(TWO_VERIFY_GAP) 
        is_link_1_valid_for_gap = await db.is_user_verified(user_id, verify_gap_seconds) 
        
        if not is_link_1_valid_for_gap:
            await query.message.edit_text("<b>⚠️ V2 verification link expired!</b>\nAapko shuru se V1 aur V2 dono complete karne honge long access ke liye.")
            return

        # V2 ke liye URL banayein (Rotation logic ke anusar next link V2 hona chahiye)
        verify_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=7))
        await db.create_verify_id(user_id, verify_id)
        temp.CHAT[user_id] = grp_id
        ...
        # Edit previous V1 message to show V2 prompt
        await query.message.edit_text(
            text=script.SECOND_VERIFICATION_TEXT.format(query.from_user.mention, get_status()),
            protect_content=False,
            reply_markup=InlineKeyboardMarkup(buttons),
            parse_mode=enums.ParseMode.HTML
        )
        ...
        logger.error(f"Error in start_v2_flow: {e}")
        # Agar koi file_id error aaya to basic error message de dein
        await query.message.edit_text(f"<b>Ek error aa gaya:</b> <code>{e}</code>")
# --- CALLBACK HANDLER KHATAM ---
    ...
    try:
        if (
            ...
            if new_count >= REFERRAL_TARGET:
                ...
                # --- FIX: Sahi DB function (db.update_user_data) ka istemaal karein ---
