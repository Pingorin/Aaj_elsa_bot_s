...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        # YEH MANTA HAI KI info.py MEIN SHORTENER_API2 = "" (empty string) HAI
        is_v2_enabled = settings.get('api_two') != SHORTENER_API2 and settings.get('api_two') != ""
    ...
    if not await db.has_premium_access(user_id):
        ...
        if not settings.get("is_verify", IS_VERIFY):
            ...
            # Check karein ki V2 enabled hai ya nahi
            # YEH MANTA HAI KI info.py MEIN SHORTENER_API2 = "" (empty string) HAI
            # Agar aapka default API2 empty nahi hai, toh check ko `settings.get('api_two') != SHORTENER_API2` rehne dein
            is_v2_enabled = settings.get('api_two') != SHORTENER_API2 and settings.get('api_two') != ""
            # --- FIX KHATAM ---
...
async def delete_all_index(bot, message):
    ...
    btn = [[
            ...
            InlineKeyboardButton(text="ᴄᴀɴᴄᴇL", callback_data="close_data")
        ]]
    if message.from_user.id not in ADMINS:
