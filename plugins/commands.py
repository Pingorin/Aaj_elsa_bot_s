...
async def start(client:Client, message): 
    ...
    # --- YEH HAI 'NOTCOPY' (AAPKE NAYE GAP ACCESS LOGIC KE SAATH) ---
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        verify_gap_seconds = settings.get('verify_gap', TWO_VERIFY_GAP) # Gap time fetch kiya
        ...
        try:
            if user_data["last_verified"] > user_data["second_time_verified"]:
                ...
                # Success Message (V2 Solved - Full Access Granted)
                ...
                key = "last_verified"
                num = 1
                readable_gap_time = get_readable_time(verify_gap_seconds)
                
                # Success Message (V1 Solved - Gap Access Granted)
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help", "buy_premium"]:
        if message.command[1] == "buy_premium":
            btn = [[
                InlineKeyboardButton('📸 sᴇɴᴅ sᴄʀᴇᴇNꜱʜᴏᴛ 📸', url=USERNAME)
            ],[
    ...
    if not await db.has_premium_access(user_id):
        ...
        # --- YEH HAI AAPKA NAYA "GAP ACCESS" LOGIC ---
        settings = await get_settings(grp_id)
        ...
        if settings.get("is_verify", IS_VERIFY):
            ...
            # STATE 2: FULL ACCESS CHECK (V2 solve ho chuka hai)
            if v2_time > v1_time:
                time_since_v2 = (current_time - v2_time).total_seconds()
                if time_since_v2 <= duration_seconds:
                    # ACCESS ACTIVE (Full). File mil jayegi.
                    show_link = False
                else:
                    # FULL ACCESS EXPIRED. V1 dikhao.
                    show_link = True
                    is_second_shortener = False # Show V1
            
            # STATE 1: GAP ACCESS CHECK (V1 solve ho chuka hai)
            elif v1_time > v2_time:
                time_since_v1 = (current_time - v1_time).total_seconds()
                if time_since_v1 <= gap_seconds:
                    # ACCESS ACTIVE (Gap). File mil jayegi.
                    show_link = False
                else:
                    # GAP ACCESS EXPIRED. V2 dikhao.
                    show_link = True
                    is_second_shortener = True # Show V2
            
            # STATE 0: NO ACCESS (Fresh user)
            else: 
                # Dono barabar hain. V1 dikhao.
                show_link = True
                is_second_shortener = False # Show V1
...
ɢᴀᴘ - `{readable_gap}` (Access time after V1)
...
# --- YEH HAI AAPKA '/set_verify_time' COMMAND (Full Access DURATION) ---
...
async def set_verify_time(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command poora nahi hai! (Yeh V2 ke baad ka 'Full Access' DURATION set karta hai)</b>\n\n"
            "Example:\n"
    ...
    reply_text = (
        f"<b>✅ 'Full Access' DURATION {title} ke liye set ho gaya hai.</b>\n\n"
...
# --- YEH HAI NAYI COMMAND '/set_verify_gap' (Gap Access DURATION) ---
...
async def set_verify_gap(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command poora nahi hai! (Yeh V1 ke baad ka 'Gap Access' DURATION set karta hai)</b>\n\n"
            "Example:\n"
    ...
    reply_text = (
        f"<b>✅ 'Gap Access' DURATION {title} ke liye set ho gaya hai.</b>\n\n"
        f"<b>Naya Gap Time (V1 aur V2 ke beech):</b> <code>{time_string}</code> ({seconds} seconds)\n\n"
    )
    
    if seconds == 0:
        reply_text += "<i>Note: Aapne <code>0</code> set kiya hai. User ko V1 ke turant baad V2 dikhaya jayega (koi Gap Access nahi).</i>"
