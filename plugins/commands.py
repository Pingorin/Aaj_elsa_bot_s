...
from info import (
    ...
    AUTH_CHANNEL, AUTH_CHANNEL_2, # <-- YAHAN AUTH_CHANNEL_2 IMPORT KAREIN
...
from utils import (
    ...
    check_fsub_status  # <-- YAHAN 'check_fsub_status' (hamara function) IMPORT KAREIN
)
import re
import json
import base64
...
async def start(client:Client, message): 
    ...
    # --- YEH HAI SAHI WALA 2-CHANNEL FSUB LOGIC ---
    if AUTH_CHANNEL:
        status_1, status_2 = await check_fsub_status(client, message.from_user.id)
        
        if status_1 in ["MEMBER", "PENDING"] and status_2 in ["MEMBER", "PENDING"]:
            pass # File dene ke liye aage badho
            ...
            if status_1 == "NOT_JOINED":
                try:
                    ...
                    fsub_buttons_row.append(
                        InlineKeyboardButton("Request Channel 1 🔗", url=link_1.invite_link)
                    )
                except ChatAdminRequired:
                    logger.error(f"Bot ko Channel 1 ({AUTH_CHANNEL}) mein admin banayein")
                except Exception as e:
            ...
            if status_2 == "NOT_JOINED":
                try:
                    ...
                    logger.error(f"Bot ko Channel 2 ({AUTH_CHANNEL_2}) mein admin banayein")
                except Exception as e:
                    logger.error(f"Channel 2 link error: {e}")

            if fsub_buttons_row:
                btn.append(fsub_buttons_row)
            ...
            if not btn:
                await message.reply_text("**Kuch error aa gaya hai.**\nBot admin nahi hai Fsub channel(s) mein.")
                return

            await message.reply_text(
                "**File lene ke liye, pehle upar diye gaye channel(s) ko join karne ki request karein.**\n\n"
                "Request bhejte hi, turant file lene ke liye **Try Again** button dabayein.",
                reply_markup=InlineKeyboardMarkup(btn)
            )
            return
    # --- FSUB LOGIC YAHAN KHATAM HOTA HAI ---
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener:
            ...
            await d.delete()
            # --- FIX: await m.delete() YAHAN SE HATA DIYA GAYA HAI ---
            return
...
async def set_shortner(c, m):
    ...
    try:
        ...
        # --- FIX: Replaced blocking 'requests' with 'aiohttp' ---
...
# --- FIX: YEH HAI SAHI COMBINED HANDLER ---
@Client.on_chat_member_updated()
async def combined_chat_member_handler(client: Client, member: ChatMemberUpdated):
    
    # --- Logic 1: FSUB Cleanup ---
    try:
        # Check karein ki event AUTH_CHANNEL ya AUTH_CHANNEL_2 ka hai
        if str(member.chat.id) in [str(AUTH_CHANNEL), str(AUTH_CHANNEL_2)]: 
            if member.new_chat_member and member.new_chat_member.user:
                ...
                # Hum user ko sirf tabhi remove karenge agar:
                # 1. Uska NAYA status 'RESTRICTED' (yaani pending) NAHI hai.
                # (Matlab ya toh woh Member ban gaya, ya Left/Banned/Dismissed ho gaya)
                
                if new_status not in [enums.ChatMemberStatus.RESTRICTED]:
                    # Check karo ki woh DB mein tha ya nahi, agar tha toh remove karo
                ...
                # Agar naya status RESTRICTED hai, toh hum kuch nahi karenge
                # (kyunki Join_req.py use abhi add karega)

    except Exception as e:
        logger.error(f"FSUB Cleanup error: {e}")
        # Return nahi karna, referral logic bhi chalna chahiye
    
    # --- Logic 2: Referral Handler (Pehle welcome_handler mein tha) ---
    try:
        if (
            member.invite_link
