...
async def combined_chat_member_handler(client: Client, member: ChatMemberUpdated):
    ...
    # --- Logic 1: FSUB Cleanup (Pehle Join_req.py mein tha) ---
    try:
        # Check karein ki event AUTH_CHANNEL ka hai
        if str(member.chat.id) == str(AUTH_CHANNEL): 
            if member.new_chat_member and member.new_chat_member.user:
                ...
                # 1. Woh pending list mein THA,
                # 2. Aur uska NAYA status 'RESTRICTED' (yaani pending) NAHI hai.
                # (Matlab ya toh woh Member ban gaya, ya Left/Banned/Dismissed ho gaya)
                
                if (
                    new_status not in [enums.ChatMemberStatus.RESTRICTED]
                    and await db.is_request_pending(user_id, channel_id)
                ):
                ...
                # (kyunki woh abhi-abhi request bhej raha hai)
