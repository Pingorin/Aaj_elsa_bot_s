...
# --- FIX: Removed old 'is_req_subscribed' and added 'get_fsub_status' ---
from utils import (
    get_settings, save_group_settings, get_size, get_shortlink, 
    is_check_admin, get_status, temp, get_readable_time, get_fsub_status
)
import re
import json
import base64
from html import escape # <-- FIX: Added for welcome_handler

# FIX 1: Initialized logger
...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith("get_referral_"):
        try:
            ...
            # --- FIX: Prevent ValueError crash if chat_id_str is empty ---
    ...
    # --- FIX: NEW ADVANCED FSUB LOGIC ---
    if AUTH_CHANNEL:
        fsub_status = await get_fsub_status(client, message.from_user.id)
        
        if fsub_status == "MEMBER" or fsub_status == "PENDING":
            # Case 1 & 2: User is Member or Pending, let them pass
            pass
        
        elif fsub_status == "NOT_JOINED":
            # Case 3: User needs to join
            try:
                # Make sure AUTH_CHANNEL is an integer
            ...
            btn = [[
                InlineKeyboardButton("⛔️ Request to Join ⛔️", url=invite_link.invite_link)
            ]]
            
            # Add "Try Again" button for deep links
            if message.command[1] != "subscribe":
                try:
                    # Try to split the command to keep the deep link data
            ...
            await client.send_message(
                ...
                text="🙁 Aapko file access karne ke liye hamara channel join karna zaroori hai.\n\n"
                     "Kripya <b>'Request to Join'</b> button dabayein. Request bhejte hi aapko file mil jayegi.",
                reply_markup=InlineKeyboardMarkup(btn),
                parse_mode=enums.ParseMode.HTML
            )
            return # Stop the user here
    # --- END OF FSUB LOGIC ---
...
# --- FIX: REPLACED ENTIRE BUGGY 'welcome_handler' WITH THE CORRECTED VERSION ---
...
async def welcome_handler(client: Client, member: ChatMemberUpdated):
    try:
        ...
        if (
            ...
            # Logic yahan se hata diya gaya hai
            ...
            if new_count >= REFERRAL_TARGET:
                ...
                # --- BUG FIX: Sahi DB update method use karein ---
                await db.update_notcopy_user(
                    referrer_id,
                    {"expiry_time": expiry_time, "referral_count": 0}  # Count reset karein
                )
                # --- FIX ENDS ---
