...
import pytz
import urllib.parse
from datetime import datetime, timedelta # <-- Timedelta Import zaroori hai
...
from info import (
    ...
    SHORTENER_API2, LOG_API_CHANNEL, 
    TWO_VERIFY_GAP,  # Default gap
    DEFAULT_VERIFY_DURATION, # Default duration (24h)
    QR_CODE, DELETE_TIME, 
...
async def start(client:Client, message): 
    ...
    # --- YEH HAI 'NOTCOPY' (SEQUENTIAL LOGIC KE LIYE UPDATED) ---
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        # Admin dwara set 'duration' (in seconds) ko fetch karein
        ...
        # Decide karein kaunsa timestamp update karna hai
        try:
            if user_data["last_verified"] > user_data["second_time_verified"]:
                # V1 > V2. Matlab V1 aakhri baar solve hua tha.
                # Iska matlab user ne abhi V2 (Link 2) solve kiya hai.
                key = "second_time_verified"
                num = 2
                
                readable_access_time = get_readable_time(verify_duration_seconds)
                # Success Message (V2 Solved - Access Granted)
                ...
                # script.SECOND_VERIFY_COMPLETE_TEXT ka istemaal karein
                caption = script.SECOND_VERIFY_COMPLETE_TEXT.format(message.from_user.mention, readable_access_time)
                caption += f"\n\n{access_msg}"

            else:
                # V1 <= V2. Matlab V2 aakhri baar solve hua tha (ya dono barabar hain).
                # Iska matlab user ne abhi V1 (Link 1) solve kiya hai.
                key = "last_verified"
                num = 1
                
                # Success Message (V1 Solved - Go to V2)
                # script.VERIFY_COMPLETE_TEXT ka istemaal karein
                caption = script.VERIFY_COMPLETE_TEXT.format(message.from_user.mention, "") # Time khali bhej do
                # Extra text hata dein jo script.py se aa raha hai
                caption = caption.replace(f"Aapko {get_readable_time(0)} ke liye free access mil gaya hai.", "").strip()
                caption = caption.replace("Aapko  ke liye free access mil gaya hai.", "").strip()
                
                caption += "\n\n<b>Step 1/2 Pura Hua!</b>\nAb agla step (V2) pura karne ke liye neeche diye gaye 'Get File' button ko dabayein."
                
        except Exception as e:
             logger.error(f"Notcopy logic error: {e}")
             key = "last_verified" # Default V1 update karo
             num = 1
             caption = "<b>Verification Pura Hua!</b>\n\nAb file lene ke liye neeche button dabayein."
        ...
        # User ko file lene ke liye "Try Again" (Get File) link bhejein
        btn = [[
    ...
    if not await db.has_premium_access(user_id):
        ...
        # --- YEH HAI AAPKA NAYA SEQUENTIAL VERIFICATION LOGIC ---
        settings = await get_settings(grp_id)
        
        if settings.get("is_verify", IS_VERIFY):
            duration_seconds = settings.get('verify_time', DEFAULT_VERIFY_DURATION)
            gap_seconds = settings.get('verify_gap', TWO_VERIFY_GAP)
            
            user_data = await db.get_notcopy_user(user_id)
            v1_time = user_data["last_verified"]
            v2_time = user_data["second_time_verified"]
            
            ist_timezone = pytz.timezone('Asia/Kolkata')
            current_time = datetime.now(tz=ist_timezone)
            
            show_link = False
            is_second_shortener = False # Default: Link 1 (V1) dikhao
            
            # Case 1: V2 solved hai (Access Check)
            if v2_time > v1_time:
                # V2 aakhri baar solve hua tha. Yeh "Access" state hai.
                time_since_v2 = (current_time - v2_time.astimezone(ist_timezone)).total_seconds()
                
                if time_since_v2 > duration_seconds:
                    # ACCESS EXPIRED. V1 dikhao (Stage 0).
                    show_link = True
                    is_second_shortener = False
                else:
                    # ACCESS ACTIVE. File mil jayegi.
                    show_link = False
            
            # Case 2: V1 solved hai (Gap Check)
            elif v1_time > v2_time:
                # V1 aakhri baar solve hua tha. Yeh "Waiting for V2" state hai.
                time_since_v1 = (current_time - v1_time.astimezone(ist_timezone)).total_seconds()
                
                if time_since_v1 >= gap_seconds:
                    # Gap (0s) pura ho gaya hai. V2 dikhao (Stage 1).
                    show_link = True
                    is_second_shortener = True
                else:
                    # User ne V1 solve kiya, lekin gap (e.g. 10s) poora nahi hua.
                    # Hum V1 hi dobara dikha denge.
                    # Note: Agar gap=0 hai (jaisa aapne kaha), toh yeh 'else' block kabhi run nahi hoga.
                    show_link = True
                    is_second_shortener = False 
            
            # Case 3: Fresh user (v1 == v2)
            else: 
                # Dono barabar hain (Fresh user). V1 dikhao (Stage 0).
                show_link = True
                is_second_shortener = False
            ...
            if show_link:
                ...
                # Button ka text badlo (1/2 ya 2/2)
                button_text = "✅️ ᴠᴇʀɪғʏ (1/2) ✅️" if not is_second_shortener else "✅️ ᴠᴇʀɪғʏ (2/2) ✅️"
                ...
                # Message text badlo
                msg = script.VERIFICATION_TEXT if not is_second_shortener else script.SECOND_VERIFICATION_TEXT
...
# ... [Baaqi saare commands (delete, settings, etc.) vaise hi rahenge] ...
...
async def deletemultiplefiles(bot, message):
    ...
    btn = [[
       ...
       InlineKeyboardButton("ɴᴏ, ᴀʙᴏʀᴛ ᴏᴘᴇʀᴀᴛɪᴏN 😢", callback_data="close_data")
    ]]
    await message.reply_text(
...
async def all_settings(client, message):
    ...
    # Readable time banayein
    readable_duration = get_readable_time(settings.get('verify_time', DEFAULT_VERIFY_DURATION))
    readable_gap = get_readable_time(settings.get('verify_gap', TWO_VERIFY_GAP))

    text = f"""<b><u>⚙️ ʏᴏᴜʀ sᴇᴛᴛɪɴɢs ꜰᴏʀ -</u> {title}

<u>✅️ 1sᴛ ᴠᴇʀɪꜰʏ sʜᴏʀᴛɴᴇʀ ɴᴀᴍᴇ/ᴀᴘɪ (V1)</u>
ɴᴀᴍᴇ - `{settings["shortner"]}`
ᴀᴘɪ - `{settings["api"]}`

<u>✅️ 2ɴᴅ ᴠᴇʀɪꜰʏ sʜᴏʀᴛɴᴇR ɴᴀᴍᴇ/ᴀᴘɪ (V2)</u>
ɴᴀᴍᴇ - `{settings["shortner_two"]}`
ᴀᴘɪ - `{settings["api_two"]}`

<u>⏰ ᴠᴇʀɪꜰʏ ꜱᴇᴛᴛɪɴɢꜱ</u>
ᴅᴜʀᴀᴛɪᴏɴ - `{readable_duration}` (Access time after V2)
ɢᴀᴘ - `{readable_gap}` (Time between V1 and V2)
...
# --- YEH HAI AAPKA '/set_verify_time' COMMAND (DURATION SET KARTA HAI) ---
...
async def set_verify_time(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command poora nahi hai! (Yeh V2 ke baad ka DURATION set karta hai)</b>\n\n"
            "Example:\n"
            "<code>/set_verify_time 6 hours</code>\n"
            "<code>/set_verify_time 1 day</code>\n"
            "<code>/set_verify_time 30 mins</code>\n"
            "<code>/set_verify_time 0</code> (Sirf 1 file ke liye access)"
        )
    ...
    reply_text = (
        f"<b>✅ Verification DURATION {title} ke liye set ho gaya hai.</b>\n\n"
        f"<b>Naya Access Time (V2 ke baad):</b> <code>{time_string}</code> ({seconds} seconds)\n\n"
    )
    
    if seconds == 0:
        reply_text += "<i>Note: Aapne <code>0</code> set kiya hai. Ab users ko har file ke liye V1 aur V2 dono solve karne padenge.</i>"
    
    await message.reply_text(reply_text)
# --- COMMAND KHATAM ---

# --- YEH HAI NAYI COMMAND '/set_verify_gap' (GAP SET KARTA HAI) ---
@Client.on_message(filters.command('set_verify_gap'))
async def set_verify_gap(client, message):
    userid = message.from_user.id if message.from_user else None
    if not userid:
        return await message.reply("<b>ʏᴏᴜ ᴀʀᴇ ᴀɴᴏɴʏᴍᴏᴜꜱ ᴀᴅᴍɪN ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ...</b>")
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command poora nahi hai! (Yeh V1 aur V2 ke beech ka GAP set karta hai)</b>\n\n"
            "Example:\n"
            "<code>/set_verify_gap 10 mins</code>\n"
            "<code>/set_verify_gap 1 hour</code>\n"
            "<code>/set_verify_gap 0</code> (V1 ke turant baad V2)"
        )
    ...
    if seconds == 0 and time_string != "0" and not time_string.startswith("0"):
         return await message.reply_text(
            "<b>Galat time format!</b>\n\n"
            "Use karein: m>min</code>, <code>hour</code>, <code>day</code>\n"
            "Example: <code>/set_verify_gap 30 mins</code>"
        )

    await save_group_settings(grp_id, 'verify_gap', seconds)
    
    reply_text = (
        f"<b>✅ Verification GAP {title} ke liye set ho gaya hai.</b>\n\n"
        f"<b>Naya Gap Time (V1 aur V2 ke beech):</b> <code>{time_string}</code> ({seconds} seconds)\n\n"
    )
    
    if seconds == 0:
        reply_text += "<i>Note: Aapne <code>0</code> set kiya hai. User ko V1 ke turant baad V2 dikhaya jayega.</i>"
