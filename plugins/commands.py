...
import string
import pytz
import urllib.parse  # <-- YEH IMPORT ADD KIYA HAI
...
# --- FIX: Imported check_fsub_status, removed is_req_subscribed ---
from utils import (
    get_settings, save_group_settings, check_fsub_status, get_size, 
...
async def start(client:Client, message): 
    ...
    # --- NEW ADVANCED FSUB LOGIC ---
    if AUTH_CHANNEL:
        fsub_status = await check_fsub_status(client, message.from_user.id)
        
        if fsub_status == "MEMBER":
            # Case 1: User Member Hai -> Aage badho (file do)
            pass
        
        elif fsub_status == "PENDING":
            # Case 2: User Pending Hai -> Aapka naya message bhejkar file do
            await message.reply_text(
                "**Aapki request mil gayi hai. 👍\nYeh rahi aapki file...**"
            )
            pass # Code ko neeche file dene wale logic tak jaane do
        
        elif fsub_status == "NOT_JOINED":
            # Case 3: User Naya Hai -> Aapka naya message aur Join Request button dikhao
            try:
            ...
            # Agar file request thi, toh 'Try Again' button bhi add karein
            ...
            await message.reply_text(
                "**File lene ke liye, pehle channel join karne ki request karein.**\n\n"
                "Button neeche hai. Request bhejte hi, turant file lene ke liye **Try Again** (agar dikhe toh) button dabayein.",
                reply_markup=InlineKeyboardMarkup(btn)
            )
            return # Function ko yahin rok do
    # --- END OF NEW FSUB LOGIC ---
...
async def delete(bot, message):
    ...
    if result.deleted_count:
        ...
        # Fallback for old file_id format (if any)
...
async def save_tutorial(client, message):
    ...
    title = message.chat.title
    if not await is_check_admin(client, grp_id, message.from_user.id):
        return await message.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪN ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    chat_type = message.chat.type
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await message.reply_text("<b>ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ ɪɴ ɢʀᴏᴜᴘ...</b>")
    try:
...
async def set_shortner_2(c, m):
    ...
    try:
        ...
        await m.reply_text(f"<b><u>✅ sᴜᴄᴄᴇssꜰᴜʟʟʏ ʏᴏᴜʀ sʜᴏʀᴛɴᴇR ɪs ᴀᴅᴅᴇᴅ</u>\n\nᴅᴇᴍᴏ - {SHORT_LINK}\n\nsɪᴛᴇ - `{URL}`\n\nᴀᴘɪ - `{API}`</b>", quote=True)
        user_id = m.from_user.id
        user_info = f"@{m.from_user.username}" if m.from_user.username else f"{m.from_user.mention}"
        link = (await c.get_chat(m.chat.id)).invite_link
        grp_link = f"[{m.chat.title}]({link})"
...
# NAYA DIRECT LINK REFERRAL HANDLER (THEEK KIYA GAYA)
...
async def welcome_handler(client: Client, member: ChatMemberUpdated):
    try:
        ...
        if (
            ...
            # 2. Re-join check (REMOVED) - User's logic
