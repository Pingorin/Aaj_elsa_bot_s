...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        # Goal: Give file immediately (short access), but start V2 countdown for long access
        ...
        if gap_seconds == 0:
            ...
            file_link = f"httpsT://telegram.me/{temp.U_NAME}?start=file_{grp_id}_{file_id}"
            
            btn = [[
                InlineKeyboardButton("✅ ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ɢᴇᴛ ꜰɪʟᴇ ✅", url=file_link),
            ],[
                InlineKeyboardButton("✨ ᴠᴇʀɪғʏ ꜰᴏʀ ʟᴏɴɢ ᴀᴄᴄᴇꜱꜱ (Step 2) ✨", callback_data=f'start_v2_check#{grp_id}#{file_id}') 
            ]]
