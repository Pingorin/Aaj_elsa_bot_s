...
from utils import (
    ...
    check_fsub_status, check_fsub_4_status  # <-- YEH FIX HAI
)
import re
import json
import base64
...
async def start(client:Client, message): 
        ...
        if fsub_4_status == "NOT_JOINED":
            ...
            try:
                ...
                btn = [
                    ...
                    [InlineKeyboardButton("Try Again ♻️", url=f"https://t.me/{temp.U_NAME}?start=file_{grp_id}_{file_id}")]
                ]
                
                await m.reply_photo(
                    photo=(VERIFY_IMG), 
                    caption=f"**Verification poora hua!**\n\nFile lene ke liye, kripya aakhri step poora karein aur neeche diye gaye channel ko join karein. Phir 'Try Again' button dabayein.",
                ...
                pass # Error aaye toh file bhej do

        # --- NAYA LOGIC KHATAM ---

        # Agar user Fsub 4 joined/pending tha, toh use file de do
    ...
    if AUTH_CHANNEL or AUTH_CHANNEL_2 or AUTH_CHANNEL_3:
        ...
        # Naya function (status1, status2, status3) return karega
        ...
        if all_joined:
            # Case 1: User ne sab join kar liya hai. File do.
            pass # Code ko neeche file dene wale logic tak jaane do
        
        else:
            # Case 2: User ne koi ek channel join nahi kiya hai.
            btn = []
            
            # Row 1: Channel 1 aur 2 ke liye
            fsub_row_1 = []
            ...
            # Row 2: Channel 3 ke liye
            fsub_row_2 = []
            
            if status_3 == "NOT_JOINED":
                try:
                    fsub_row_2.append(
                        InlineKeyboardButton("Join Channel 3 🔗", url=AUTH_CHANNEL_3_INVITE_LINK)
                    )
                except Exception as e:
                    logger.error(f"Channel 3 link error: {e}")
            
            if fsub_row_2:
                btn.append(fsub_row_2)

            # 'Try Again' button add karein
