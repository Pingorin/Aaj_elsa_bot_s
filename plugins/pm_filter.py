...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # --- UPDATED: SET EARNING MODE HANDLER ---
    ...
    # --- NEW: TOGGLE DISABLE/ENABLE ---
        ...
        # Agar Shortlink ON hai, to hume use Disable karna hai
        if is_shortlink_on:
            # Check Fsubs
            fsub_set = False
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        if total_results >= 3:
            btn.insert(0,[InlineKeyboardButton("♻️ sᴇɴᴅ ᴀʟʟ", callback_data=batch_link),InlineKeyboardButton("🥇ʙᴜʏ🥇", url=f"https://t.me/{temp.U_NAME}?start=buy_premium"),InlineKeyboardButton("📰 ʟᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}#0#{req}")])
        else:
    ...
    if offset != "":
        BUTTONS[key] = search
        req = message.from_user.id if message.from_user else 0
        btn.append([InlineKeyboardButton(text=f"1/{math.ceil(int(total_results) / int(MAX_BTN))}", callback_data="pages"),InlineKeyboardButton(text="ɴᴇxᴛ ⪼", callback_data=f"next_{req}_{key}_{offset}")])
        key = f"{message.chat.id}-{message.id}"
        BUTTONS[key] = search
        req = message.from_user.id if message.from_user else 0
        try: offset = int(offset) 
