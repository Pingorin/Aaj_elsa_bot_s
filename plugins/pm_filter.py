...
# --- STATE MANAGEMENT FOR FSUB SETTINGS ---
temp.FSUB_WAITING = {} 
# ------------------------------------------

# --- NEW LISTENER: Handles Channel ID Input for FSub Settings ---
...
async def fsub_input_handler(client, message):
    ...
    if user_id in temp.FSUB_WAITING:
        ...
        # Validation (Simple check if it looks like an ID)
        if not (chat_id_text.startswith("-100") and chat_id_text[1:].isdigit()):
            ...
            # Stop here, don't propagate to search
        ...
        # Readable Slot Name
        slot_map = {
            'fsub_id_1': 'Request Fsub Slot 1',
            'fsub_id_2': 'Request Fsub Slot 2',
            'fsub_id_3': 'Normal Fsub Slot 3',
            'fsub_id_4': 'Post-Verify Fsub Slot 4'
        }
        slot_name = slot_map.get(slot_key, slot_key)

        await message.reply_text(f"✅ <b>{slot_name} has been set for chat {grp_id}.</b>\n\nChannel ID: `{channel_id}`")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # --- 1. REQUEST FSUB MENU (UPDATED UI) ---
        ...
        text = (
            ...
            f"✅ <b>Slot 1:</b> {name_1}\n"
            f"✅ <b>Slot 2:</b> {name_2}\n"
            f"✅ <b>Slot 4 (Post-Verify):</b> {name_4}\n\n"
            f"<i>Click 'Edit' to change or 'Clear' to remove a slot.</i>"
        )
        ...
        btn = [
            ...
            [InlineKeyboardButton("🚫 Remove All Fsub Channels", callback_data=f"remove_all_fsub#{grp_id}#req")],
            [InlineKeyboardButton("🔙 Back to Fsub Settings", callback_data=f"fsub_menu#{grp_id}")]
        ]
        
        await query.message.edit(text, reply_markup=InlineKeyboardMarkup(btn))

    # --- 2. NORMAL FSUB MENU (UPDATED UI) ---
        ...
        if id_3:
            try:
                ...
                chat_name = f"{chat.title} ({id_3})"
            except:
                chat_name = f"Unknown/Error ({id_3})"
        
        text = (
            f"<b>⚙️ Configure Normal F-Sub (Auth 3) for:</b> `{grp_id}`\n\n"
            f"✅ <b>Slot 3:</b> {chat_name}\n\n"
            f"<i>Click 'Edit' to set new or 'Clear' to remove.</i>"
        )
        ...
        btn = [
            ...
            [InlineKeyboardButton("🚫 Remove All Fsub Channels", callback_data=f"remove_all_fsub#{grp_id}#norm")],
            [InlineKeyboardButton("🔙 Back to Fsub Settings", callback_data=f"fsub_menu#{grp_id}")]
        ]
        await query.message.edit(text, reply_markup=InlineKeyboardMarkup(btn))
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        if total_results >= 3:
            ...
            btn.insert(0,[InlineKeyboardButton("🥇ʙᴜʏ🥇", url=f"https://t.me/{temp.U_NAME}?start=buy_premium"),InlineKeyboardButton("📰 ʟᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}#0#{req}")])
    else:
        if total_results >= 3:
            btn.insert(0,[InlineKeyboardButton("♻️ sᴇɴᴅ ᴀʟʟ", callback_data=batch_link),InlineKeyboardButton("🥇ʙᴜʏ🥇", url=f"https://t.me/{temp.U_NAME}?start=buy_premium")])
        else:
    ...
    if offset != "":
        BUTTONS[key] = search
        req = message.from_user.id if message.from_user else 0
        btn.append([InlineKeyboardButton(text=f"1/{math.ceil(int(total_results) / int(MAX_BTN))}", callback_data="pages"),
             InlineKeyboardButton(text="ɴᴇxᴛ ⪼", callback_data=f"next_{req}_{key}_{offset}")])
        key = f"{message.chat.id}-{message.id}"
        BUTTONS[key] = search
        req = message.from_user.id if message.from_user else 0
        try: offset = int(offset) 
        except: offset = int(MAX_BTN)
