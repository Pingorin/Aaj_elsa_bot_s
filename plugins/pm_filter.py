...
# --- ⚠️ IMPORTANT ⚠️ ---
# Yahaan Bot 2 ka username (bina @) daalein
...
async def pm_search(client, message):
    if IS_PM_SEARCH:
        # (Language check waise hi rahega)
...
async def group_search(client, message):
    # (Group search logic Bot 1 ke liye zaroori hai, yeh waise hi rahega)
...
async def next_page(bot, query):
    ...
    # temp.FILES_ID (batch) logic Bot 1 mein zaroori nahi, lekin rakh sakte hain
    batch_ids = [file.link_id for file in files] # Batch ke liye link_id use karein
    ...
    if settings["link"]:
        ...
        for file_num, file in enumerate(files, start=offset+1):
            links += f"""<b>\n\n{file_num}. <a href=https://t.me/{BOT_2_USERNAME}?start=get_{file.link_id}>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}</a></b>"""
    else:
        btn = [[InlineKeyboardButton(
                    text=f"🔗 {get_size(file.file_size)}≽ {get_name(file.file_name)}", 
                    url=f'https://telegram.dog/{BOT_2_USERNAME}?start=get_{file.link_id}'
                ),]
                for file in files
              ]
    # --- BADLAAV KHATAM ---
    ...
    btn.insert(0,[
        ...
        InlineKeyboardButton("🥇ʙᴜʏ🥇", url=f"https://t.me/{BOT_2_USERNAME}?start=buy_premium"), # Bot 2 ko point karein
    ...
    # Bot 1 ko Bot 2 ke referral link par point karna chahiye
    btn.append(
        [InlineKeyboardButton("💰 ʀᴇꜰᴇʀ & ᴇᴀʀɴ 💰", url=f"https://t.me/{BOT_2_USERNAME}?start=get_referral_{query.message.chat.id}")]
    )
    btn.append(
    ...
    if settings["link"]:
        # (Link mode ke liye links dobara generate karein - user ke code mein bug tha)
        links = "" # Reset links
        for file_num, file in enumerate(files, start=offset+1):
            links += f"""<b>\n\n{file_num}. <a href=https://t.me/{BOT_2_USERNAME}?start=get_{file.link_id}>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}</a></b>"""
...
async def quality_filter_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not available_qualities or len(available_qualities) < 2:
        return await query.answer("No other qualities found to filter.", show_alert=True)
...
async def set_quality_cb_handler(client: Client, query: CallbackQuery):
    # (Yeh function waise hi rahega, lekin file links update honge)
    try:
    ...
    if not files:
        ...
        # ... (code to show quality menu again) ...
        return
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not available_years or len(available_years) < 2:
        return await query.answer("No other years found to filter.", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    # (Bot 1 ke liye simplified cb_handler)
    if query.data == "close_data":
        ...
        # (Admin command, Bot 1 mein rahega)
    ...
    # --- BADLAAV: Stream logic Bot 1 se hata diya ---
    # (elif query.data.startswith("stream"): ... HATA DIYA)
    ...
    # --- Start Menu Logic (Bot 1 mein rahega) ---
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(text=script.EARN_TEXT.format(temp.B_LINK), reply_markup=reply_markup, parse_mode=enums.ParseMode.HTML)
        ...
        await query.message.reply_photo(photo=(QR_CODE), caption=script.PREMIUM_TEXT, reply_markup=reply_markup, parse_mode=enums.ParseMode.HTML)

    # --- Admin/Settings Logic (Bot 1 mein rahega) ---
    elif query.data == "all_files_delete":
        files = await Media.count_documents() # Note: Yeh sabhi DBs ka count nahi karega, sirf default ka.
        await query.answer('Deleting...')
        # Yeh sirf default DB (MediaSecondary) ko drop karega.
        await Media.collection.drop() 
        await query.message.edit_text(f"Successfully deleted {files} files from default collection")
        
    elif query.data.startswith("killfilesak"):
        # (Admin command, Bot 1 mein rahega)
        ident, keyword = query.data.split("#")
        await query.message.edit_text(f"<b>Fetching files for {keyword} on DB...</b>")
        files, total = await get_bad_files(keyword)
        await query.message.edit_text(f"<b>Found {total} files for {keyword}!</b>")
        deleted = 0
        async with lock:
            try:
                for file in files:
                    file_ids = file.file_id
                    # Yeh sabhi DBs se delete karne ki koshish karega (agar get_bad_files sabse laata hai)
                    # Lekin 'file.file_id' ab search_id hai, 'link_id' nahi.
                    # Yeh logic complex ho sakta hai. Behtar hai ise Bot 2 mein rakhein.
                    # Abhi ke liye, hum maan rahe hain ki get_bad_files() sahi kaam kar raha hai.
                    result = await Media.collection.delete_one({'_id': file_ids}) # Sirf default DB se delete karega
                    if result.deleted_count: deleted += 1
                    if deleted % 20 == 0:
                        await query.message.edit_text(f"<b>Deleted {str(deleted)} files...</b>")
            except Exception as e:
                await query.message.edit_text(f'Error: {e}')
            else:
                await query.message.edit_text(f"<b>Successfully deleted {str(deleted)} files.</b>")
          
    elif query.data.startswith("reset_grp_data"):
        # (Group settings, Bot 1 mein rahega)
        grp_id = query.message.chat.id
        ...
        await query.message.edit_text("<b>ꜱᴜᴄᴄᴇꜱꜱғᴜʟʟʏ ʀᴇꜱᴇᴛ ɢʀᴏᴜᴘ ꜱᴇᴛᴛɪɴɢꜱ...</b>", reply_markup=InlineKeyboardMarkup(btn))
        ...
        if not await is_check_admin(client, int(grp_id), userid):
            ...
            return
        if status == "True":
            await save_group_settings(int(grp_id), set_type, False); await query.answer("ᴏғғ ❌")
        else:
            await save_group_settings(int(grp_id), set_type, True); await query.answer("ᴏɴ ✅")
    ...
    # --- BADLAAV: Request handling logic (show_options se ..._alert tak) hata diya ---
        ...
        # --- BADLAAV (BOT 1) ---
        link = f"https://telegram.me/{BOT_2_USERNAME}?start=allfiles_{group_id}-{message_id}"
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        if total_results >= 3:
            ...
            btn.insert(0,[
                InlineKeyboardButton("🥇ʙᴜʏ🥇", url=f"https://t.me/{BOT_2_USERNAME}?start=buy_premium"),
                InlineKeyboardButton("📰 ʟᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}#0#{req}")
            ])
    else:
        if total_results >= 3:
    ...
    btn.append(
        [InlineKeyboardButton("💰 ʀᴇꜰᴇʀ & ᴇᴀʀɴ 💰", url=f"https://t.me/{BOT_2_USERNAME}?start=get_referral_{message.chat.id}")] # Bot 2 ko point karein
    )
    btn.append(
        [InlineKeyboardButton("🤔 ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ 🤔", url=settings['tutorial'])]
    )
...
async def advantage_spell_chok(message):
    ...
    settings = await get_settings(chat_id)
    query = re.sub(
        r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|((send|snd|giv(e)?|gib)(\sme)?)|movie(s)?|new|latest|br((o|u)h?)*|^h(e|a)?(l)*(o)*|mal(ayalam)?|t(h)?amil|file|that|find|und(o)*|kit(t(i|y)N?)?o(w)?|thar(u)?(o)*w?|kittum(o)*|aya(k)*(um(o)*)?|full\smovie|any(one)|with\ssubtitle(s)?)",
    ...
    await asyncio.sleep(120); await d.delete(); await message.delete()
