...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        # --- FIX: Replaced 'is_req_subscribed' with 'get_fsub_status' ---
        if AUTH_CHANNEL:
            fsub_status = await get_fsub_status(client, query.from_user.id)
            if fsub_status == "NOT_JOINED":
                await query.answer("ɪ ʟɪᴋᴇ ʏᴏᴜʀ sᴍᴀʀᴛɴᴇss ʙᴜᴛ ᴅᴏɴ'ᴛ ʙᴇ ᴏᴠᴇʀsᴍᴀᴛ 😒\nꜰɪʀsᴛ ᴊᴏɪɴ ᴏᴜʀ ᴜᴘᴅᴀᴛᴇs ᴄʜᴀɴɴᴇʟ 😒", show_alert=True)
                return
        # --- END FIX ---
