...
async def next_page(bot, query):
    ...
    if settings["link"]:
        ...
        for file_num, file in enumerate(files, start=offset+1):
            # --- BOT 1 FIX: Point to BOT_2_USERNAME ---
            links += f"""<b>\n\n{file_num}. <a href=https://t.me/{BOT_2_USERNAME}?start=file_{query.message.chat.id}_{file.file_id}>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}</a></b>"""
    else:
        # --- BOT 1 FIX: Point to BOT_2_USERNAME ---
        btn = [[InlineKeyboardButton(text=f"🔗 {get_size(file.file_size)}≽ {get_name(file.file_name)}", url=f'https://telegram.dog/{BOT_2_USERNAME}?start=file_{query.message.chat.id}_{file.file_id}'),]
                for file in files
              ]
    btn.insert(0,[
        InlineKeyboardButton("♻️ sᴇɴᴅ ᴀʟʟ", callback_data=batch_link),
        InlineKeyboardButton("🥇ʙᴜʏ🥇", url=f"https://t.me/{temp.U_NAME}?start=buy_premium"), # Keep 'Buy' pointing to Bot 1
    ...
    # FIX: Add 'Refer & Earn' Button (Points to Bot 1)
    btn.append(
...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    if settings["link"]:
        ...
        # --- BOT 1 FIX: Point to BOT_2_USERNAME (and fixed original bug) ---
        btn = [[
                InlineKeyboardButton(text=f"🔗 {get_size(file.file_size)}≽ {get_name(file.file_name)}", url=f'https://telegram.dog/{BOT_2_USERNAME}?start=file_{query.message.chat.id}_{file.file_id}'),]
                   for file in files
              ]
...
async def set_quality_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not files:
        ...
        buttons.append([InlineKeyboardButton(text="⪻ ʙᴀᴄᴋ ᴛᴏ ᴍᴀɪN ᴘᴀɢᴇ", callback_data=f"next_{req}_{key}_{original_offset}")])
...
async def set_year_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not files:
        ...
        buttons.append([InlineKeyboardButton(text="⪻ ʙᴀᴄᴋ ᴛᴏ ᴍᴀɪɴ ᴘᴀɢᴇ", callback_data=f"next_{req}_{key}_{original_offset}")])
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # --- BOT 1 FIX: Remove 'stream' logic ---
    # This bot doesn't send files, so it doesn't need to generate stream links.
    ...
    #     ...
        ...
        # This is an admin command, can be kept in Bot 1
        ...
        async with lock:
            try:
                for file in files:
                    ...
                    # --- BOT 1 FIX: Bot 1 should only delete from DBs it manages (if any) ---
                    # Assuming Bot 1 and Bot 2 share DBs, this is fine.
                    result = await Media.collection.delete_one({ '_id': file_ids, })
                    if result.deleted_count:
                        print(f'Successfully deleted {file_name} from database.')
                    deleted += 1
                    if deleted % 20 == 0:
                        await query.message.edit_text(f"<b>Process started... Successfully deleted {str(deleted)} files...</b>")
            except Exception as e:
                print(e)
                await query.message.edit_text(f'Error: {e}')
            else:
                await query.message.edit_text(f"<b>Successfully deleted {str(deleted)} files from database.</b>")
          
    elif query.data.startswith("reset_grp_data"):
        # Group settings are needed for Bot 1
        grp_id = query.message.chat.id
        ...
        await query.message.edit_text("<b>ꜱᴜᴄᴄᴇꜱꜱғᴜʟʟʏ ʀᴇꜱᴇᴛ ɢʀᴏᴜᴘ ꜱᴇᴛᴛɪɴɢꜱ...</b>", reply_markup=reply_markup)
    ...
    # --- BOT 1 FIX: Remove all request handling logic ---
    # Bot 1 doesn't handle requests, Bot 2 does.
    # elif query.data.startswith("show_options"): ...
    # elif query.data.startswith("reject"): ...
    # elif query.data.startswith("accept"): ...
    # ... all other request-related 'elif' blocks ...
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["link"]:
        ...
        for file_num, file in enumerate(files, start=1):
            # --- BOT 1 FIX: Point to BOT_2_USERNAME ---
            links += f"""<b>\n\n{file_num}. <a href=https://t.me/{BOT_2_USERNAME}?start=file_{message.chat.id}_{file.file_id}>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}</a></b>"""
    else:
        # --- BOT 1 FIX: Point to BOT_2_USERNAME ---
        btn = [[InlineKeyboardButton(text=f"🔗 {get_size(file.file_size)}≽ {get_name(file.file_name)}", url=f'https://telegram.dog/{BOT_2_USERNAME}?start=file_{message.chat.id}_{file.file_id}'),]
               for file in files
              ]
    if offset != "":
        if total_results >= 3:
