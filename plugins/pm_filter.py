...
from info import (
    ...
    SHORTENER_API, SHORTENER_WEBSITE2, SHORTENER_API2, 
    
    # --- YEH V3 IMPORT ADD KIYA GAYA ---
    SHORTENER_WEBSITE3, SHORTENER_API3,
    # --- IMPORT KHATAM ---
    
    IS_PM_SEARCH, QR_CODE, DELETE_TIME, 
    REFERRAL_TARGET
)
...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    # --- YEH HAI FIX ---
    # Dono missing buttons add kiye gaye
    btn.append(
        [InlineKeyboardButton("💰 ʀᴇꜰᴇʀ & ᴇᴀʀɴ 💰", url=f"https://t.me/{temp.U_NAME}?start=get_referral_{query.message.chat.id}")]
    )
    btn.append(
