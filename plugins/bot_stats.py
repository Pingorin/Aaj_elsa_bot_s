    ...
    if temp.ME in check:
        if (str(message.chat.id)).startswith("-100") and not await db.get_chat(message.chat.id):
            ...
            try:
                ...
                group_link = "N/A (Bot ko admin banayein)"
...
async def groups_list(bot, message):
    ...
    async for chat in chats:
        try:
            ...
            members_count = chat_info.members_count if chat_info.members_count else "N/A"
...
async def get_ststs(bot, message):
    ...
    db4_size_str = get_size(db_stats.get('db4_size', 0))
    ...
    await message.reply_text(script.STATUS_TXT.format(
        ...
        size, 
        free,
        db_stats.get('db1_files', 'N/A'), 
        db1_size_str,
        db_stats.get('db2_files', 'N/A'), 
        db2_size_str,
        db_stats.get('db3_files', 'N/A'), 
        db3_size_str,
        db_stats.get('db4_files', 'N/A'), 
        db4_size_str,
        uptime, 
        ram, 
        cpu
