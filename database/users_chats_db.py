...
class Database:
    ...
    async def is_user_verified(self, user_id, time_gap_seconds):
        ...
        if time_gap_seconds == 0:
            # --- YEH HAI ASLI FIX (15 Second Grace Period) ---
            # Yeh V1 -> V2 -> FSub-4 flow ko '0' par bhi chalne dega
            return total_seconds <= 15 
            # --- FIX KHATAM ---
    ...
    async def use_second_shortener(self, user_id, time): # 'time' yahan 'button_gap_seconds' (TWO_VERIFY_GAP) hai
        ...
        if is_link_1_valid or is_link_2_valid:
            ...
            # Yahan 'time' 0 seconds hai aur 15s grace period hai.
            # Agar user ne V1 ya V2 15s ke andar kiya hai, toh False return karo (link mat dikhao)
            return False

        # Agar hum yahan hain, matlab *dono* links 15s se zyada puraane hain.
