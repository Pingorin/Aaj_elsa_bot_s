...
class Database:
    ...
    async def is_user_verified(self, user_id, time_gap_seconds):
        ...
        # Ab yahan long-term access time_gap_seconds ke hisaab se check ho raha hai.
    ...
    async def has_premium_access(self, user_id):
        ...
        if user_data:
            ...
            if expiry_time is None:
                return False
            # FIX: datetime.datetime.now() ko timezone-aware banaya
            elif isinstance(expiry_time, datetime.datetime) and datetime.datetime.now(pytz.utc) <= expiry_time.astimezone(pytz.utc):
                return True
            else:
