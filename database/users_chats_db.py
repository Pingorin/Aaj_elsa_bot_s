...
class Database:
    ...
    async def is_user_verified(self, user_id, time_gap_seconds):
        ...
        if time_gap_seconds == 0:
            ...
            # Ise 15 se 300 kar diya gaya hai (5 minutes)
    ...
    async def use_second_shortener(self, user_id, time): # 'time' yahan 'button_gap_seconds' (TWO_VERIFY_GAP) hai
        ...
        # Yeh main expiry (24 ghante) nahi hai.
        # Hum check kar rahe hain ki V2, V1 se purana hai ya nahi (grace period ke hisaab se)
        is_link_2_valid = await self.is_user_verified_second(user_id, time)

        if is_link_2_valid:
            # User verified hai (V2 link active hai). Koi link mat dikhao.
            # Yahan 'time' 0 seconds hai aur 300s grace period hai.
            # Agar user ne V2 300s ke andar kiya hai, toh False return karo (link mat dikhao)
            return False
