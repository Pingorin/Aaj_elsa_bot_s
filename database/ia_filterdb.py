...
# --- YEH HAIN NAYE STATS FUNCTIONS ---

async def get_db_size_primary():
    try:
        return (await mydb_primary.command("dbstats"))['dataSize']
    except Exception:
        return 0

async def get_db_size_secondary():
    try:
        return (await mydb_secondary.command("dbstats"))['dataSize']
    except Exception:
        return 0

async def get_db_size_third():
    try:
        return (await mydb_third.command("dbstats"))['dataSize']
    except Exception:
        return 0

async def get_files_count_primary():
    try:
        return await MediaPrimary.count_documents()
    except Exception:
        return 0

async def get_files_count_secondary():
    try:
        return await MediaSecondary.count_documents()
    except Exception:
        return 0
        
async def get_files_count_third():
    try:
        return await MediaThird.count_documents()
    except Exception:
        return 0

# Purana function (ab /stats mein use nahi hoga)
async def get_files_db_size():
    try:
        return (await mydb_secondary.command("dbstats"))['dataSize']
    except Exception:
        return 0 
# --- STATS FUNCTIONS KHATAM ---
