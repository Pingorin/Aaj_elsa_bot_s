...
from info import DATABASE_URI2, DATABASE_NAME, COLLECTION_NAME, MAX_BTN, QUALITIES
...
async def get_search_results(query, max_results=MAX_BTN, offset=0, lang=None, quality=None, year=None):
    ...
    # BUG FIX: Removed the inefficient 'if lang:' block that loaded all results into memory.
    # The language is now part of the main 'query' (e.g., "search movie hindi"),
    # so it's correctly handled by the efficient database-side pagination.
...
async def get_available_qualities(query):
    ...
    # We will check up to 200 files for qualities to balance performance and accuracy.
    ...
    async for file in cursor:
        ...
        for quality in QUALITIES:
            # Use regex to find quality as a whole word or with 'p'
    ...
    return sorted(list(available_qualities), reverse=True) # e.g., ["4K", "1080p", "720p"]

async def get_available_years(query):
    # This regex finds 4-digit numbers starting with 19 or 20
    ...
    # Check up to 200 files for years
    cursor = Media.find(filter).limit(200) 
    ...
    return sorted(list(available_years), reverse=True) # Newest years first
