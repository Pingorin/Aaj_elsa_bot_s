...
import asyncio
# import shortzy  <-- Removed duplicate import
...
# --- FIX: REMOVED OLD/BUGGY `is_req_subscribed` FUNCTION ---

# --- ADDED NEW, CORRECT `check_fsub_status` FUNCTION ---
async def check_fsub_status(bot, user_id):
    """
    API का उपयोग करके AUTH_CHANNEL में यूज़र का स्टेटस चेक करता है।
    यह डेटाबेस पर निर्भर नहीं करता और रेस कंडीशन को फिक्स करता है।
    Returns: "MEMBER", "PENDING", "NOT_JOINED"
    """
    try:
        member = await bot.get_chat_member(AUTH_CHANNEL, user_id)
        
        # Case 1: यूज़र मेंबर है (या एडमिन/ओनर)। फाइल दो।
        if member.status in [enums.ChatMemberStatus.MEMBER, enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER]:
            return "MEMBER"
        
        # Case 2: यूज़र RESTRICTED है (मतलब उसकी 'Join Request' पेंडिंग है)। फाइल दो।
        if member.status == enums.ChatMemberStatus.RESTRICTED:
            return "PENDING"
        
        # Case 3: यूज़र LEFT या BANNED है (मतलब उसे किक/dismiss किया गया था)। फाइल मत दो।
        return "NOT_JOINED"
        
    except UserNotParticipant:
        # Case 4: यूज़र चैनल में किसी भी तरह से नहीं है।
        # (मतलब: वह नया है, या उसकी रिक्वेस्ट 'Dismiss' हुई, या उसने खुद 'Cancel' की)
        # किसी भी हालत में, फाइल मत दो।
        return "NOT_JOINED"
        
    except Exception as e:
        # Case 5: कोई और एरर (जैसे बॉट एडमिन नहीं है)। सेफ्टी के लिए, फाइल मत दो।
