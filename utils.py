...
import re
import os 
# from shortzy import Shortzy # Shortzy ka istemaal nahi ho raha hai
...
#--- YEH HAI AAPKE 'get_shortlink' FUNCTION KA NAYA FIX (V3 ke liye) ---
async def get_shortlink(link, grp_id, step_number=1):
    """
    step_number=1 -> V1 Shortener
    step_number=2 -> V2 Shortener
    step_number=3 -> V3 Shortener
    """
    ...
    if step_number == 3:
        api_key, site_key = 'api_three', 'shortner_three'
    elif step_number == 2:
        api_key, site_key = 'api_two', 'shortner_two'
    else: # Default Step 1
        api_key, site_key = 'api', 'shortner'

    api, site = settings.get(api_key), settings.get(site_key)        
    
    if not api or not site:
        logger.warning(f"Shortener API/Site (Step {step_number}) settings nahi mili.")
        return link
        
    # Link ko encode karein
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url) as resp:
                if resp.status != 200:
                    logger.error(f"Shortener HTTP Error {resp.status} for {site} (Step {step_number})")
                    return link 
        ...
        if data.get('status') == 'success':
            ...
            logger.error(f"Shortener API Error: {error_message} for {site} (Step {step_number})")
            return link 
            
    except Exception as e:
        logger.error(f"Aiohttp error in get_shortlink (Step {step_number}): {e}")
        return link
